...
1. `GenerateUUID` 后直接保留其真实返回值作为本次唯一 `case_id`（可加固定 `case-` 前缀，但不得改写为日期/序号、截短或丢弃 UUID）。一个案件内唯一且永不复用。
2. 用 `Ls` / `Glob` 清点用户提交的全部文件，并对这组**当前提交且可读的精确文件路径**优先调用一次 `FileDigest`。仅一份文件时，`paths` 必须是该文件的完整绝对裸路径字符串，不得传数组 JSON 文本；多份文件时，`paths` 必须是完整集合的原生字符串数组。先按 `${SKILL_DIR}/../../inventory/o0-input-inventory.schema.json` 建立闭合的分类库存，再逐份登记其真实冻结元组：
