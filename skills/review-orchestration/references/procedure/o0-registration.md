...
9. **实际调用 `Skill` 装载 `coverage-matrix`**，由它作为唯一协议建立初始矩阵；`default_enabled` 或记得其模板都不等于已调用。在首次矩阵输出前，还必须从已装载技能的实际 `${SKILL_DIR}` 分别 `Read` `references/coverage-matrix-bucket-summary.schema.json`、`references/coverage-matrix-bucket-summary.descriptor.json` 与 `references/coverage-matrix-baseline.catalog.json`，并核其真实 pin；不能用记忆、旧任务摘要或自写副本替代。完成规则源预检后，调用 `coverage-matrix-baseline-init`：`catalog_paths` 必须恰为已实际 Read 的 `${SKILL_DIR}/references/coverage-matrix-baseline.catalog.json`、当前团队 `shared/resources/jurisdiction-packs/base/missing-clauses.yaml`、`base/market-benchmarks.yaml`、`custom/pack.yaml`、`custom/redlines.yaml`，以及仅 `read_and_pinned` 的 resolved 法域同次 Read `shared/resources/jurisdiction-packs/<selected-pack>/rules.yaml`；所以未定法域为 5 个、已解析法域为 6 个 snapshot。当前 custom 模板已加载但零 enabled 项，不能改写为“缺席”或另造 custom 行。`output_path` 只能是既有 Lead 父目录下 create-only 的 `<workspace>/contract-review/coverage-matrix.yaml`。`case_id`、`generated_at`、`rules_path`、`pack_version` 都只是声明，必须由 Lead 紧接着以此前 exact Read 比对；不得把脚本回显当成案件、规则源或法律事实认证。`clarification_required` 只传现有 typed pending，绝不传 guessed pack。脚本成功后实际 `Read` exact 输出、比较 resolved `rules[]`/`conflicts[]` 的每个唯一 ID 与 stage 4 行集合双向相等，并以既有 schema `StructuredFileValidate` 验证；脚本/Read/比较/SFV 任一失败均 `O0_COVERAGE_MATRIX_INVALID` HOLD。不得先判断适用性/trigger，不得漏冲突条目、接受未知动态 catalog、沿用空模板的 summary，或自行另造 rows/状态/算术规则。
