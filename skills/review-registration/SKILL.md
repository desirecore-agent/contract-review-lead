...
1. 首次 `Write` 前，实际 `Read` `review-context/review-context.schema.json` 与 `review-context/review-context.template.yaml`；补充登记还可 `Read` 已有的同案 context。`Read` 不得用于合同正文或附件。任一允许读取的文件不可读、不可解析或不一致则停止并如实说明，不能自由编写替代 YAML。按 schema/template 的闭合枚举、required、`allOf` 写入完整 context；写后必须 `Read` 回核，不把“成功写入”当成 schema 合规。
