...
test('Lead routes bounded registration to its dedicated skill while full orchestration retains context handoff requirements', async () => {
  ...
  assert.equal(leadFrontmatter.version, '1.0.18')
  assert.equal(leadFrontmatter.metadata.version, '1.0.18')
  assert.equal(coverageFrontmatter.version, '1.0.8')
  assert.equal(coverageFrontmatter.metadata.version, '1.0.8')
  ...
  assert.equal(agent.version, '1.0.26')
  ...
  assert.match(lead, /用户逐字给出的绝对路径直接复用，不得自动改写；当前团队 cwd 已确认且用户给出不含 `\.\.` 的相对路径（可含子目录）时，直接原样作为 `Read` \/ `FileDigest` 参数/)
  assert.match(lead, /由工具按 context\.cwd 和既有路径安全校验解析/)
  assert.match(lead, /由工具按 context\.cwd 和既有路径安全校验解析，不得先拼接长 cwd/)
  assert.match(lead, /不得猜测、截断、重组\/换根路径，也不得以旧 personal workspace 替代/)
  assert.match(lead, /某一明确路径错误只能记录该路径错误，不得推导所有目录未授权/)
  assert.match(lead, /仅一份文件时，`paths` 必须是该文件原样的裸字符串；绝对路径原样保留，已确认 cwd 下不含 `\.\.` 的相对路径（可含子目录）也原样传入/)
  ...
  assert.match(lead, /成功时逐项核对 `files\[\]\.path` 对应原始提交集合、`files\[\]` 完整且 `aggregate\.file_count` 等于集合数；只用工具返回的 `absolute_path` 和 `digest` 登记规范绝对路径与摘要，不把它声称为自动 realpath 身份/)
